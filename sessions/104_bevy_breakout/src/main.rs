...
const PADDLE_SIZE: Vec2 = Vec2::new(60.0, 10.0);
...
const BRICK_SIZE: Vec2 = Vec2::new(50., 15.);
// These values are exact
const GAP_BETWEEN_PADDLE_AND_BRICKS: f32 = 100.0; //270.0
...
fn check_for_collisions(
    ...
    mut collider_query: Query<(Entity, &Transform, &mut Sprite, Option<&mut Brick>), With<Collider>>,
    mut collision_events: EventWriter<CollisionEvent>,
) {
    let (mut ball_velocity, ball_transform) = ball_query.into_inner();

    for (collider_entity, collider_transform, mut collider_sprite, maybe_brick) in &mut collider_query {
        ...
        if let Some(collision) = collision {
            ...
            if let Some(mut brick) = maybe_brick {
                // Debug
                // println!("{:?}", brick);
                // println!("{:?}", collider_sprite);
                brick.0 -= 1;
                if brick.0 == 0 {
                    commands.entity(collider_entity).despawn();
                    **score += 1;
                } else {
                    let color_map = [Color::srgb(0.5, 1.0, 0.5),Color::srgb(1.0,0.5,0.5)];
                    collider_sprite.color = color_map[brick.0-1];
                };
            }
